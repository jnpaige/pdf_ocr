# pdf_ocr

This pipeline converts a corpus of PDFs into structured, machine-readable outputs for downstream LLM use, including RAG indexing, generating wikis describing the characteristics of those PDFs, and full-text search. Each PDF is processed in two complementary passes: Surya OCR reads the raw page imagery (handling both printed text and handwriting), and Docling uses that recognized text alongside its own layout detection to recover document structure, including section headers, tables, and reading order, and exports clean Markdown. The result per document is a Markdown file suitable for chunking and embedding, a searchable PDF with an invisible text layer, and supporting JSON and plain-text outputs.

Every tool in this ecosystem takes pdf_ocr output as its starting input. The downstream tools that ingest the products of pdf_ocr are [site_form_segmenter](https://github.com/jnpaige/site_form_segmenter), which maps out the structure of each document according to categories of interest, [text_coding_program](https://github.com/jnpaige/text_coding_program), a browser-based interface to help people perform text coding across that corpus, [Site_coder](), a set of scripts for applying locally run large language models to text coding tasks, and [Site_wiki](), which generates wiki style pages based on the contents of pdfs by synthesizing that information with large language models. More broadly, pdf_ocr is meant to serve as first point in any one of several different kinds of methods of turning a textual corpus into structured datasets, semistructured data, or translating that unstructured information into a new kind of unstructured information, whether assisted by deep learning, regular expressions, or to augment the kinds of more traditional coding tasks people perform. 

---

## Output

For each input PDF a subfolder is created containing four files. The plain text file (`text_docling.txt`) is a dump of each page separated by `=== Page N ===` markers, with heading items prefixed by markdown hash markers. The Markdown file (`<pdf_stem>.md`) uses the identical `=== Page N ===` page delimiting, but each page's content is real Docling markdown (tables reconstructed, headings, reading order) where Docling's layout model successfully parsed that page, or the same raw OCR text `text_docling.txt` has for that page — wrapped in a labeled code fence — on pages where it didn't (common on handwriting-dense corpora; both files draw from the identical underlying per-page text either way, `.md` just additionally carries table/heading structure where available). The searchable PDF (`<pdf_stem>_ocr.pdf`) is the original PDF with an invisible Surya text layer added, making it fully text-searchable and selectable, plus the same `=== Page N ===` marker stamped as real visible/selectable text top-right of every page so a page in the PDF can be matched back to its `.md`/`.txt` section by eye or by copy-paste; this file is omitted when `docling.do_ocr` is set to false. The structured JSON file (`ocr_docling.json`) contains per-page results with text blocks, confidence scores, and a bounding box per line, letting any downstream tool locate a piece of extracted text back to its position on the original page (see `locate_and_annotate.py`, below). Each page also carries a `tables` list: for every table Docling recovered, its page region, grid dimensions, a bounding box per cell (with row/column indices and header flags), and a unioned bounding box per row — so a downstream tool can address an individual table row on the page rather than only the prose around it. **Every bounding box in this file uses a `BOTTOMLEFT` origin.** Docling itself is inconsistent here (text provenance is bottom-left, table cells are top-left), so boxes are normalized on write; `origin` is still recorded on every box and consumers should keep branching on it rather than assuming, since a box that could not be converted reports its true origin instead of being mislabelled.

The downstream tools each consume different output files. The `text_docling.txt` file feeds into [site_form_segmenter](https://github.com/jnpaige/site_form_segmenter) for page classification, and into site_coder, site_attribute_extractor, and site_vocab_extractor for LLM prompt text — the `.md` file works as a drop-in alternative input for the same tools wherever table reconstruction is worth the cost of a full OCR rerun to get it (see the GLO workflow below for a worked example of both a segmenter and an extractor consuming `.md` instead). The `.md` file also feeds into site_wiki for extraction and synthesis. The `_ocr.pdf` feeds into [text_coding_program](https://github.com/jnpaige/text_coding_program) for the PDF page viewer, into site_form_segmenter in vision mode for contact sheet rendering, and into this repo's own `locate_and_annotate.py` as the base layer for a visual-QA overlay of extracted mentions.

Below I outline a few different downstream workflows that can feed off of the pdf_ocr foundation. 

---

## Downstream workflow 1: Site forms

Site forms are short documents, typically 2–15 pages, each filed under one trinomial. The trinomial is often somewhere in the source filename in this particular corpus and each form may contain multiple investigations appended together. Across investigations, interpretations and findings differ. Consequently, gathering standardized data about each investigation across a large corpus of these PDFs (thousands) requires isolating which pages relate to each investigation, and then routing only those investigation's pages to whatever LLM or human coder is doing the extraction. That page segmentation also makes LLM performance more predictable (and enables using more lightweight LLMs) by avoiding the noise of sending irrelevant pages alongside the information you want to extract.

The [site_form_segmenter](https://github.com/jnpaige/site_form_segmenter) handles this first. It runs a one or multiple LLM prompts using information from each document to identify page chunks that relate to particular concepts. For example, we might identify investigation boundaries, then classifying each investigation's pages into, say, form pages, narrative pages, and NRHP eligibility pages. The output is a structured page map stored as a `.segments.json` file for each site. All downstream tools read that map and limit their inputs to the pages it specifies.

We might then want to extract structured information from each investigation's relevant pages. An LLM assisted workflow in achieving this is an interesting possibility, though the error and hallucination rate of a given model in any given extraction task is an important thing to understand. Furthermore, we may want to have stronger control and understanding over the biases of a given model (we may want the outputs of an LLM to adhere very closely to the kinds of standards a human would employ in coding. To try to achieve both a method of assessing model performance, and to help pull LLM coding decisions closer to what we expect, we employ a codebook driven prompting strategy. [site_coder]() uses the parsed codebook entries directly as LLM prompts, so LLMs receive instructions analogous to what you would give human coders. That enables a direct comparison with archaeologists or other expert coders on the same coding tasks. The [text_coding_program](https://github.com/jnpaige/text_coding_program) produces output in the same JSON schema as site_coder, enabling inter-rater reliability analysis among people and models working from the same documents and the same codebook.


---

## Downstream workflow 2: Archaeological reports

Reports are longer documents, typically 50–400+ pages, that discuss multiple sites. Unlike site forms, the trinomial is not the filename. One report may cover anywhere from one to hundreds of sites. Pulling standardized information about each site from a large report corpus requires a more careful segmentation approach, along with other ways to narrow down the pages of interest.  

In this workflow, the [site_form_segmenter](https://github.com/jnpaige/site_form_segmenter) runs first on the whole document to classify pages into structural sections: executive summary, methods, results, and recommendations. Rather than sending full page text, impractical for reports that can be over 500 pages long, this pass sends only the heading map produced by pdf_ocr: a compact list of heading text and page numbers, typically a few hundred lines regardless of document length. Each heading entry also has a short excerpt of the first body text on that page so the model can distinguish real section starts from table-of-contents listings. Even after this segmentation step, the total number of pages of interest could be in the hundreds. To narrow those pages down we need to collect information about how many different concepts of interest (in this case, trinomials) are discussed in the report. Here, we use site_vocab_extractor which can be implemented to identify all trinomials and alternative site numbers mentioned in the report. Its output includes a `source_file` column that links each discovered trinomial back to its source report, preventing cross-contamination when you later feed those trinomial lists to the second segmentation pass.

The second segmentation pass then iterates per trinomial: for each site the vocab extractor identified, it searches the relevant section pages (from pass 1) for mentions of that specific site. The result is a per-trinomial page map scoped to just the report sections and pages that discuss that site. Feeding all trinomials to a single prompt might work for short reports with few sites, but archaeological reports vary enormously in length and site count, making that approach unpredictable at scale. Per-trinomial iteration isolates errors and keeps context focused at the cost of more LLM calls, but each call is short and predictably sized.

Once per-trinomial page maps exist, the same downstream tools as in the site form workflow handle extraction: site_coder, site_attribute_extractor, site_wiki, and [text_coding_program](https://github.com/jnpaige/text_coding_program).

---

## Downstream workflow 3: GLO survey field notes

GLO (General Land Office) survey field-note volumes are a third document shape, distinct from both site forms and reports. A single volume is typically 300–430 pages and records section-line surveys for a dozen or more distinct Township/Range grid cells back-to-back, each block running anywhere from a handful of pages to over a hundred, further subdivided into individual Sections. Extracting anything meaningful (tree species named as witness/bearing trees, roads, fields, villages, evidence of Indigenous presence) requires first knowing which pages belong to which Township/Range/Section — the same "segment before you extract" principle as the other two workflows, applied to a spatial grid instead of investigations or sites.

[site_form_segmenter](https://github.com/jnpaige/site_form_segmenter) handles this with two interchangeable segmenters producing the identical output schema — see its README's "GLO survey field note segmentation" section for the full comparison and why the LLM-based one exists (in short: a regex-only approach was checked against real page text and found to silently merge several distinct townships into one label wherever a volume switched header formats partway through, which a fixed pattern can't generalize past but an LLM reading the actual sentence can).

Once a `segments.json` exists (from either segmenter), [site_vocab_extractor](https://github.com/jnpaige/site_vocab_extractor) runs in `iterate_segments` mode: one LLM call per Township/Range/Section unit, tagging every extracted mention with the page it came from (validated against the literal `=== Page N ===` markers the model was shown — see that repo's README for how this stays hallucination-safe) rather than location being inferred after the fact. Two parallel prompts cover different concept sets on the same corpus — `extract_ecological_terms.txt` (tree species, understory, terrain/hydrology, land quality, fire/burning) and `extract_built_environment_terms.txt` (transportation, agricultural/field, settlement/structure, Indigenous presence) — both consuming the same segmentation and producing the same `item/unit_label/.../key/value/page` CSV shape.

The final step, `locate_and_annotate.py` in this repo, is the one piece of this workflow that lives in pdf_ocr rather than downstream: an optional visual-QA pass that takes any of those extraction CSVs, fuzzy-matches each `value` against the real OCR cells on its self-reported `page` (falling back to a search across the whole unit's pages if that page doesn't actually contain a match), and draws the result as colored, clustered boxes directly on that document's `_ocr.pdf` — one color per extraction category, grouped into a small marker grid per paragraph so several categories or several mentions in one block of text don't visually stack on top of each other. This is deliberately decoupled from the extraction tools themselves: nothing about running it is required, and any tool that already writes the shared `item/unit_label/.../key/value` CSV shape can feed it, not just the two GLO prompts above.

---

## Run metadata standard

The tools that turn pdf_ocr output into LLM-derived data — [site_form_segmenter](https://github.com/jnpaige/site_form_segmenter) (both its site-form and report segmenters), [site_coder](), and site_vocab_extractor all write their run output in the same self-documenting shape. Each output is provided in a folder which supplies information about wwhich model produced this file, when, with what prompt, and under what chunking strategy, and applied to what corpus. 

Every model run creates its own folder, `runs/<YYYYMMDD_HHMM>_<gitsha>/`. Every output filename is prefixed with the model (or model pair) that produced it: `<model_slug>__<item>.<ext>`. If a tool reprocesses the same item with a different model, that produces a second, distinct file.

The contents of the run folders are as follows:

| File | Purpose |
|---|---|
| a config snapshot (original filename preserved) | the exact settings used for this run |
| `prompts.yaml` | full text of every prompt used |
| `run_metadata.json` | run-level record: `run_id`, `started_at`/`completed_at`, `elapsed_seconds`, the model(s) used, a `chunking` block describing the chunking/context strategy in effect, per-item timings, and token statistics |
| `inventory.csv` | one row per output file which records: `run_id, tool, model, file_name, file_path, source_input, prompt_file, prompt_snapshot_key, temperature, num_ctx, chunk_strategy, produced_at, output_file_path` |

Each output file also embeds its own model, timestamp, and chunking fields directly in its JSON (exact field names vary slightly by tool — see each tool's README for its schema), so a single file is self-describing even outside its run folder.


---

## Installation

I use uv for Python environment management. It is far faster than conda or pip for snapping a repo to the right environment, it works identically on Windows, Mac, and Linux, and it never requires manually activating a virtual environment.

```powershell
# Windows — install uv once per machine
winget install astral-sh.uv

# Then in the repo directory (creates .venv and installs deps in seconds):
uv sync

# Run the pipeline
uv run python run.py
```

```bash
# Mac/Linux — install uv once per machine
curl -LsSf https://astral.sh/uv/install.sh | sh

# Then in the repo directory:
uv sync
uv run python run.py
```

`uv.lock` is committed to the repo, so every machine gets identical package versions. If you prefer to activate the venv manually, run `.venv\Scripts\activate` on Windows or `source .venv/bin/activate` on Mac/Linux, then use `python run.py` directly.

For GPU acceleration, which is recommended for large corpora:

```powershell
uv pip install torch --index-url https://download.pytorch.org/whl/cu128
```

## Usage

Set `pdf_input` and `output_dir` in `config.yaml`, then run:

```powershell
uv run python run.py

# or with a custom config:
uv run python run.py --config path/to/config.yaml
```

## Configuration

`config.example.yaml` in the repo root is an annotated reference covering every setting, with the values used for the 2026-08 corpus runs (2024 site forms, Phase II reports, GLO survey field notes). Copy it to `config.yaml` and edit. Only `pdf_input` and `output_dir` are required.

```yaml
pdf_input:  'C:\path\to\pdfs'     # single PDF or directory of PDFs
output_dir: 'C:\path\to\output'   # subfolders created per PDF

chunk_size: 50                    # split PDFs longer than this; see below

docling:
  do_ocr: true                    # run Surya OCR (default)
  # markdown_dir: 'C:\path\to\md' # optional: all .md files mirrored here (flat)
```

| Setting | Default | Purpose |
|---|---|---|
| `pdf_input` | — | A single `.pdf`, or a directory of them (non-recursive) |
| `output_dir` | — | One subfolder per PDF, named after its stem |
| `chunk_size` | unset | Split PDFs longer than N pages, OCR each piece, merge results |
| `skip_existing` | `false` | Skip PDFs whose output already has `ocr_docling.json` |
| `force_reprocess` | `false` | Redo everything; overrides `skip_existing` if both are set |
| `start_from` | unset | Skip stems sorting before this value (also `--start-from`) |
| `file_list` | unset | Process only the PDFs named in this file (also `--file-list`) |
| `from_ocr_pdf` | `false` | Read `<name>/<name>_ocr.pdf` from subfolders instead of `*.pdf` |
| `docling.do_ocr` | `true` | Run Surya OCR over page imagery |
| `docling.markdown_dir` | unset | Mirror every `.md` into one flat directory |
| `docling.extract_figures` | `false` | Extract embedded figures as images |
| `docling.figures_scale` | `2.0` | Resolution multiplier for extracted figures |
| `docling.classify_figures` | `false` | Run Docling's picture classifier |
| `docling.delete_figure_classes` | unset | Delete figures classified into these labels instead of saving them |

**Figures.** With `extract_figures: true`, each PDF gets a `figures/` subfolder of `fig_p<page>_<index>.png` crops plus a `figures.json` manifest. Beyond `page`/`bbox`/`classification` (the last only if `classify_figures: true`), each entry also carries `caption` (the figure's own printed caption, if Docling found one on the page), `heading_path` (the section-heading stack active at that point in the document), and `text_before`/`text_after` (snippets of the surrounding body text, not the caption itself). Together these make each figure a self-contained unit — image plus everything a reader would use to interpret it — that a downstream VLM pass can prompt from without re-reading the whole document. On Windows, `classify_figures: true` currently needs the environment variable `TORCHDYNAMO_DISABLE=1` set before running — Docling's picture classifier calls `torch.compile`, which requires Triton (not available on Windows) unless compilation is disabled this way.

**Deleting non-figure noise.** Docling's picture classifier (`docling-project/DocumentFigureClassifier-v2.5`) routinely flags a third or more of "figures" extracted from journal PDFs as publisher chrome — a masthead banner repeated on every page (`logo`), or the CrossRef "Check for updates" badge (`icon`) — not real content. Set `delete_figure_classes: ['logo', 'icon']` to delete these at extraction time rather than saving them; verified by hand against a real corpus that both labels are reliable before trusting them as grounds for deletion. Docling's full label set also includes `page_thumbnail`, `qr_code`, `bar_code`, `stamp`, `signature`, `screenshot_from_computer`, `screenshot_from_manual`, `calendar`, `crossword_puzzle`, `music` if a corpus needs the wider net. `lib/ocr_docling.py`'s `prune_figures_by_class()` is the shared function behind this — it also works retroactively against a `figures.json` from a run that predates this setting, without re-running OCR.

**Figure captioning.** Docling's built-in picture-description stage sends figure image data to an outside server, so it is walled off here and its config keys are rejected at startup. Use the `figure_captioner` repo instead — it captions the extracted figures with a model running on this machine.

**Chunking.** `chunk_size: 50` is what the 2026-08 runs used. It applies only to documents that exceed it, so one value is safe across a mixed corpus — 2–15 page site forms never chunk, while 185-page Phase II reports and 380-page GLO volumes split into 4 and 8 pieces respectively. Page numbering stays continuous across chunk boundaries, so `=== Page N ===` markers still match the original document; intermediate pieces are kept in a `_chunks/` subfolder. Without it, a long document is one slow pass that loses everything if it crashes.

**Flat markdown.** When `markdown_dir` is set, every `.md` is also written there with no subfolders — useful for pointing a vector store or RAG indexer at a single location.

**Reading an existing text layer.** Set `do_ocr: false` when the input PDFs already carry one, such as `_ocr.pdf` outputs from a previous run of this pipeline (usually paired with `from_ocr_pdf: true`). Docling parses that layer instead of running Surya, and no new `_ocr.pdf` is written. This is much cheaper but **not equivalent**: re-parsing an existing text layer has produced character-interleaved corruption in `text_docling.txt` (`"Site Name Site Name"`) on a corpus where a fresh `do_ocr: true` pass was clean. Since `text_docling.txt` is what every downstream tool treats as ground truth, spot-check the actual text against a known-good baseline rather than only confirming the expected files and fields exist.

One YAML gotcha: `docling:` with all sub-keys commented out parses as null rather than an empty mapping, which crashes on startup. Keep at least one sub-key live — `do_ocr: true` states the default explicitly and is the simplest choice.

## Dependencies

- [PyMuPDF](https://pymupdf.readthedocs.io/) — PDF reading and invisible text layer construction
- [Docling](https://github.com/DS4SD/docling) — layout detection, table extraction, Markdown export
- [Surya OCR](https://github.com/datalab-to/surya) — OCR backend (text detection + recognition, including handwriting)
- [PyYAML](https://pyyaml.org/) — config parsing
- [Transformers](https://huggingface.co/docs/transformers/) — required by Surya models
