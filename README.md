...
A single site form may contain 1–5 investigations appended chronologically. The segmenter identifies where each investigation starts and which pages belong to it. A aage map reduces amount of information passes on to LLMs. Instead of sending the entire form to the LLM, each tool sends only the pages relevant to the current investigation and page type. A 12-page form with 3 investigations might send 2–4 pages per LLM call, which helps us incorporate smaller local LLMs into the worflow. A key goal for an open science approach to this kind of thing. 
...
For now we approach this with a two-stage segmentation. Pass 1 identifies the report's structural sections (fast, one call on the whole document). Pass 2 identifies per-trinomial pages within those sections (iterates per trinomial, scoped to the section pages from pass 1). The resulting map, pages from relevant sections that discuss the trinomial in question, should help keep each LLM call manageable.

Iteration beats all-at-once here. Feeding all trinomials to a single prompt forces the model to hold the full text and full trinomial list and produce structured output for each. This would be ok if there are small site counts, and short reports. But that is unpredictable across site reports in archaeology. Per-trinomial iteration isolates errors and keeps context focused, and should produce more predictable outputs, at the cost of expanding the number of runs. 
